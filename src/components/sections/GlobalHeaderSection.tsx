...
interface GlobalHeaderSectionProps {
...
/**
 * GlobalHeaderSection - Global navigation header
 * Migrated from GlobalHeader.tsx for better structure organization
 * Optimized for performance and accessibility
 */
export const GlobalHeaderSection: React.FC<GlobalHeaderSectionProps> = ({
  onNavClick,
  onLogoClick,
  className = '',
  'data-testid': testId = 'global-header',
}) => {
  ...
  return (
    <header 
      className={`fixed top-0 z-[100] w-full h-16 sm:h-18 lg:h-20 flex items-center justify-between px-3 sm:px-4 lg:px-20 bg-[#1e1e1e]/95 backdrop-blur-md border-b border-gray-700/30 ${className}`}
      data-testid={testId}
    >
      ...
      <div
        className={`absolute top-16 sm:top-18 lg:top-20 left-0 right-0 bg-[#1e1e1e]/95 backdrop-blur-md transition-all duration-300 ease-in-out ${
          isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'
        } lg:hidden border-t border-gray-700/30 shadow-lg z-[90]`}
      ...
      >
        <nav className="flex flex-col py-3">
          ...
          <div className="flex flex-col gap-2 mt-2 p-2 border-t border-gray-700/30">
            <div className="mb-2">
              <LanguageToggle
                currentLanguage={currentLanguage}
                onLanguageChange={switchLanguage}
...
export default GlobalHeaderSection;